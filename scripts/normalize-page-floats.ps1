$root = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path

$standard = @'
      <aside class="page-floats" aria-label="바로가기">
        <a href="#" class="page-floats__online">온라인<br>전시회</a>
        <button type="button" class="page-floats__top" data-scroll-top aria-label="맨 위로">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
            <path d="M12 5l7 7H5l7-7z" fill="currentColor" />
          </svg>
        </button>
      </aside>
'@

$asidePattern = '(?s)\s*<aside class="[a-z0-9_-]+-floats" aria-label="바로가기">.*?</aside>'

Get-ChildItem -Path $root -Recurse -Filter '*.html' -File |
  Where-Object { $_.Name -notmatch '~$' } |
  ForEach-Object {
    try {
      $content = [IO.File]::ReadAllText($_.FullName)
    } catch {
      Write-Warning "Skip: $($_.FullName)"
      return
    }
    $newContent = [regex]::Replace($content, $asidePattern, "`n$standard")
    if ($newContent -ne $content) {
      [IO.File]::WriteAllText($_.FullName, $newContent)
      Write-Host "Updated: $($_.FullName)"
    }
  }
